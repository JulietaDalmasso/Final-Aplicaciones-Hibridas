...
export async function getTodosProyectos(filtros = {}) {
  await client.connect()
  return await db.collection('Projects').find(filtros).toArray()
}

export async function getProyectosByUsuario(usuarioId, filtros = {}) {
  await client.connect()
  const proyectos = await db.collection('Projects').find({
    owner: new ObjectId(usuarioId),
    ...filtros
  }).toArray()
  return proyectos
}
...
export async function agregarColaborador(id, usuarioId, email) {
  ...
  if (String(proyecto.owner) !== String(usuarioId)) throw new Error('Solo el owner puede invitar')
  ...
  await db.collection('Projects').updateOne(
    { _id: new ObjectId(id) },
    {
      $addToSet: { colaboradores: normalizado },
...
export async function quitarColaborador(id, usuarioId, email) {
  ...
  const proyecto = await db.collection('Projects').findOne({ _id: new ObjectId(id) })
  if (!proyecto) throw new Error('Proyecto no encontrado')
  if (String(proyecto.owner) !== String(usuarioId)) throw new Error('Solo el owner puede eliminar')
  ...
  await db.collection('Projects').updateOne(
    { _id: new ObjectId(id) },
    {
      $pull: { colaboradores: normalizado },
      $set: { updatedAt: new Date() }
    }
  )
  return true
